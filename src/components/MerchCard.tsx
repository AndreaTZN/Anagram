...
export default function MerchCard({ name, price, src }: Props) {
  ...
  useGSAP(
    () => {
      gsap.set(addToBagRef.current, { width: 0 });
  ...
  function onEnter() {
    gsap.to(addToBagRef.current, {
      width: "7.5rem",
      duration: 0.4,
      ease: "power3.out",
    });
    gsap.to(addToBagTextRef.current, {
      opacity: 1,
      duration: 0.2,
      ease: "power2.out",
      delay: 0.15,
    ...
    >
      ...
      <div className="absolute bottom-[1.47rem] left-1/2 -translate-x-1/2 flex items-center w-[calc(100%-3rem)]">
        ...
        <div className="flex-1 min-w-0 flex items-center justify-between px-[1.625rem] py-[1.25rem] rounded-full backdrop-blur-[2.5rem] bg-[rgba(12,12,12,0.2)]">
          <span className="font-medium text-xs leading-[0.9] text-white opacity-50 whitespace-nowrap">
            {name}
          </span>
          <span className="font-medium text-xs leading-[0.9] text-white whitespace-nowrap">
            {price}
          </span>
        </div>
        ...
        >
          <p
            ref={addToBagTextRef}
            className="font-medium text-xs leading-[0.9] text-[#0c0c0c] whitespace-nowrap"
          >
            Add to bag
          </p>
        </div>
