...
export default function TeamCredits({
  ...
  useLayoutEffect(() => {
    ...
    gsap.to(el.querySelectorAll(".tc-primary"), { color: dark ? "#ffffff" : "#0c0c0c", duration, ease: "power2.inOut" });
    gsap.to(el.querySelectorAll(".tc-muted"), { color: dark ? "#9e9e9e" : "#7c7c7c", duration, ease: "power2.inOut" });
  }, [activeTab]);

  return (
    <div ref={containerRef} className="teamcredits_component flex gap-4 items-start w-full">
        ...
        <div className="teamcredits_list flex flex-col gap-4">
          {rows.map((row, i) => (
            <div key={i} className="teamcredits_row flex flex-col gap-1">
              <p className="tc-primary text-[#0c0c0c] text-[0.875rem] leading-[0.9]">{row.role}</p>
              <p className="tc-muted text-[#7c7c7c] text-[0.875rem] leading-[1.3]">{row.names}</p>
            </div>
          ))}
        </div>
      </div>
